...
void testRandom(const int numCoord, const double L, const int ngrid, const double sigma, unsigned seed) {
  ...
  std::cout << "energy_GSEk = " << energy_GSEk << std::endl;
  cpuGSEk.interpolateForce(numCoord, xyzq, forceX, forceY, forceZ);
  saveForce("forceGSEk.txt",numCoord, forceX, forceY, forceZ);
...
#define USE_LES
  ...
  printf("max(curlB) = %e\n",cpuLES.maxCurlB());
  printf("max(curlE) = %e\n",cpuLES.maxCurlE());
  cpuLES.getRho().save("rhoLES.txt");

  cpuLES.interpolateForceVW(sigma/sqrt(2.0), 3.0, numCoord, xyzq, forceX, forceY, forceZ);
  saveForce("forceLES.txt",numCoord, forceX, forceY, forceZ);
#endif
  
  //--------------------------------------------------------------------------------------------------------  
  // Ewald
  //--------------------------------------------------------------------------------------------------------
  double* refX = new double[numCoord];
  double* refY = new double[numCoord];
  double* refZ = new double[numCoord];
  CpuEwaldRecip cpuEwald(sigma, 12, boxx, boxy, boxz);
  double energy_ew = cpuEwald.calcForceEnergy(numCoord, xyzq, refX, refY, refZ);
  std::cout << "energy_ew = " << energy_ew << std::endl;
  //printf("EWALD: %e %e %e | %e %e %e\n",refX[0],refY[0],refZ[0],refX[1],refY[1],refZ[1]);
  //printf("rms = %e\n",Erms(numCoord, refX, refY, refZ, forceX, forceY, forceZ));
...
void testPair(const double r, const double L, const int ngrid, const double sigma) {
  ...
  xyzq[0].y = 0.5*boxy + 0.5;
  xyzq[0].z = 0.5*boxz + 0.5;
  xyzq[1].x = r/2.0 + 0.5*boxx;
  xyzq[1].y = 0.5*boxy + 0.5;
  xyzq[1].z = 0.5*boxz + 0.5;
...
#define USE_LES
  ...
  cpuLES.initElectricField();
  cpuLES.getRho().save("rhoLES.txt");
  cpuLES.getEx().save("ExLES0.txt");
  cpuLES.getEy().save("EyLES0.txt");
  cpuLES.getEz().save("EzLES0.txt");
  double err = cpuLES.checkGaussLaw();
  printf("Error in Gauss Law = %e\n",err);
  for (int i=0;i < 1000;i++) {
    cpuLES.integrate(2.0, 0.1, 1.0);
    //printf("max(curlB), max(curlE) = %e %e\n",cpuLES.maxCurlB(),cpuLES.maxCurlE());
  }
  double energy_LES = cpuLES.calcTotalEnergy();
  ...
  //cpuLES.interpolateForce(cpuGSEk.getSigma1(), cpuGSEk.getLambdaSigma1(), 2, xyzq, forceX, forceY, forceZ);
  ...
  cpuLES.getEx().save("ExLESd.txt");
  cpuLES.getEy().save("EyLESd.txt");
  cpuLES.getEz().save("EzLESd.txt");
