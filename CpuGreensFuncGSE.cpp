...
//
template <typename T>
void CpuGreensFuncGSE<T>::mul(const int nfftX, const int nfftY, const int nfftZ,
  ...
  for (int iz=0;iz < sizeZ;iz++) {
	...
	T fac = fourpi_vol*exp(-sigmafac*kd2)/kd2;
	rhoData[p]   *= fac;
	rhoData[p+1] *= fac;
      }
    }
...
//
// Explicit instances of CpuGreensFuncGSE
//
template class CpuGreensFuncGSE<float>;
template class CpuGreensFuncGSE<double>;
